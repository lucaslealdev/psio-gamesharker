...
begin
  // Obt�m o caminho completo para o arquivo MENU.SYS
  CaminhoArquivo := ExtractFilePath(Application.ExeName) + NomeArquivo;

  // Verifica se o arquivo MENU.SYS existe
  if not FileExists(CaminhoArquivo) then
  begin
    // O arquivo n�o foi encontrado, exibe uma mensagem e fecha o programa
...
// Fun��o principal para popular a ListBox1 com os subdiret�rios
...
var
  ...
  TipoArquivo: string;
begin
  // Obt�m o caminho do diret�rio onde o execut�vel est�
  Caminho := ExtractFilePath(Application.ExeName);

  // Limpa a Form1.ListBox1 antes de adicionarmos os subdiret�rios
  ...
  // Inicia a busca por subdiret�rios
  if FindFirst(Caminho + '*.*', faDirectory, SearchRec) = 0 then
  begin
    try
      repeat
        // Ignora diret�rios especiais ('.' e '..')
        if (SearchRec.Name <> '.') and (SearchRec.Name <> '..') then
        begin
          // Verifica se � um diret�rio
          if (SearchRec.Attr and faDirectory) = faDirectory then
          begin
            // Verifica se o diret�rio possui pelo menos um arquivo do tipo desejado
            if DirectoryHasFilesOfType(Caminho + SearchRec.Name, TiposArquivoDesejados) then
            begin
              // Adiciona o nome do subdiret�rio � Form1.ListBox1
...
begin
  ...
  try
    // Abre o arquivo em modo Rewrite para sobrescrever o conte�do existente
    ...
    begin
      // Procura por arquivos com a extens�o desejada
      if FindFirst(Diretorio + '\*.' + TiposArquivo[I], faAnyFile, SearchRec) = 0 then
      begin
        try
          repeat
            // Ignora diret�rios especiais ('.' e '..') e apenas adiciona arquivos
    ...
    // Adiciona uma linha final contendo o primeiro arquivo encontrado no diret�rio selecionado no ListBox1 com caminho relativo
    ...
    if not PrimeiroArquivo then
      ...
      // Procura pelo primeiro arquivo nas extens�es permitidas no diret�rio selecionado
      for I := Low(TiposArquivo) to High(TiposArquivo) do
      begin
...
var
  ...
  TiposArquivoDesejados: array of string;
  I: Integer;
  AlgumItemSelecionado: Boolean; // Vari�vel para verificar se algum item est� selecionado
begin
  // Obt�m o caminho do diret�rio onde o execut�vel est�
  Caminho := ExtractFilePath(Application.ExeName);

  // Define os tipos de arquivo desejados (IMG, BIN, ISO) em mai�sculo e min�sculo
  SetLength(TiposArquivoDesejados, 6);
  TiposArquivoDesejados[0] := 'IMG';
  TiposArquivoDesejados[1] := 'BIN';
  TiposArquivoDesejados[2] := 'ISO';

  // Verifica se algum item est� selecionado na ListBox1
  ...
  // Se nenhum item estiver selecionado, exibe uma mensagem e sai da fun��o
  ...
  for I := 0 to ListBox1.Items.Count - 1 do
  begin
    // Cria o arquivo MULTIDISC.LST no diret�rio correspondente
